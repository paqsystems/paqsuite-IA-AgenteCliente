CREATE OR ALTER PROCEDURE dbo.PAQ_Stock_PartidaMovimiento
...
BEGIN
    ...
    -- Verificar columnas de join STA09
    ...
    SET @tCompExpr = CASE
        WHEN @hasTCompS20 = 1 THEN N'COALESCE(NULLIF(LTRIM(RTRIM(sta20.T_COMP)), ''''), sta14.T_COMP)'
        ELSE N'sta14.T_COMP'
    END;
    SET @nCompExpr = CASE
        WHEN @hasNCompS20 = 1 THEN N'COALESCE(NULLIF(LTRIM(RTRIM(sta20.N_COMP)), ''''), sta14.N_COMP)'
        ELSE N'sta14.N_COMP'
    END;
    ...
    EXEC sp_executesql @sqlTotal,
        N'@p_fd DATETIME, @p_fh DATETIME, @p_ca NVARCHAR(20), @p_cd NVARCHAR(20),
          @p_tm NVARCHAR(1), @p_np NVARCHAR(40)',
        @p_fd = @fecha_desde, @p_fh = @fecha_hasta,
        @p_ca = @cod_articu, @p_cd = @cod_deposi,
        @p_tm = @tipo_mov, @p_np = @nro_parti;
    ...
    SET @sqlPaged = N'
        ...
        ' + @joinSta11 + N'
        WHERE ' + @where + N'
        ORDER BY ' + @fechaExpr + N' ASC, ' + @codArticuExpr + N' ASC
        OFFSET @p_offset ROWS FETCH NEXT @p_page_size ROWS ONLY';

    EXEC sp_executesql @sqlPaged,
        N'@p_fd DATETIME, @p_fh DATETIME, @p_ca NVARCHAR(20), @p_cd NVARCHAR(20),
          @p_tm NVARCHAR(1), @p_np NVARCHAR(40), @p_emp NVARCHAR(100),
