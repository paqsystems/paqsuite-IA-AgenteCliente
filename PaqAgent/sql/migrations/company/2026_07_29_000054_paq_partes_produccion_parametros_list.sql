CREATE OR ALTER PROCEDURE dbo.PAQ_PartesProduccion_ParametrosList
...
BEGIN
    ...
    BEGIN
        ...
        SELECT
            CAST(NULL AS NVARCHAR(100)) AS Programa,
    ...
    DECLARE @p_programa NVARCHAR(100) = N'PartesProduccion';

    DECLARE @countSql NVARCHAR(MAX) = N'
        SELECT COUNT(*) AS total_filas
        FROM dbo.PQ_PARAMETROS_GRAL
        WHERE LOWER(CAST(' + @programaQuoted + N' AS NVARCHAR(100))) COLLATE DATABASE_DEFAULT
            = LOWER(@p_programa) COLLATE DATABASE_DEFAULT
          AND ISNULL(CAST(' + @claveQuoted + N' AS NVARCHAR(100)), N'''') <> N''''';

    -- RS0: total de filas (sin paginación; consistencia dual-RS)
    EXEC sp_executesql
        @countSql,
        N'@p_programa NVARCHAR(100)',
        @p_programa = @p_programa;

    DECLARE @sql NVARCHAR(MAX) = N'
        SELECT
            ISNULL(CAST(' + @programaQuoted + N' AS NVARCHAR(100)), N'''') AS Programa,
