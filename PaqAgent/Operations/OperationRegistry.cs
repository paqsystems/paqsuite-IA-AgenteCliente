...
public class OperationRegistry
{
    ...
    private Dictionary<string, IOperationHandler> BuildHandlers()
    {
        ...
        foreach (var (name, definition) in _settings.Definitions)
        {
            ...
            if (string.Equals(name, AcopiosAsociacionCreateOperation.OperationKey, StringComparison.OrdinalIgnoreCase))
            {
                handlers[name] = new AcopiosAsociacionCreateOperation(
                    name,
                    definition.StoredProcedure,
                    definition.Connection,
                    _sqlExecutor,
                    _loggerFactory.CreateLogger<AcopiosAsociacionCreateOperation>());
                continue;
            }

            if (string.Equals(name, AcopiosAsociacionDeleteOperation.OperationKey, StringComparison.OrdinalIgnoreCase))
            {
                handlers[name] = new AcopiosAsociacionDeleteOperation(
                    name,
                    definition.StoredProcedure,
                    definition.Connection,
                    _sqlExecutor,
                    _loggerFactory.CreateLogger<AcopiosAsociacionDeleteOperation>());
                continue;
            }

            if (string.Equals(name, AcopiosFacturaDetalleGetOperation.OperationKey, StringComparison.OrdinalIgnoreCase))
            {
                handlers[name] = new AcopiosFacturaDetalleGetOperation(
                    name,
                    definition.StoredProcedure,
                    definition.Connection,
                    _sqlExecutor,
                    _loggerFactory.CreateLogger<AcopiosFacturaDetalleGetOperation>());
                continue;
            }

            if (string.Equals(name, AcopiosPedidoDetalleGetOperation.OperationKey, StringComparison.OrdinalIgnoreCase))
            {
                handlers[name] = new AcopiosPedidoDetalleGetOperation(
                    name,
                    definition.StoredProcedure,
                    definition.Connection,
                    _sqlExecutor,
